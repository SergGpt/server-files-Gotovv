    ...
    for (let key in tuningParams) {
        if (tuningParams[key].hasOwnProperty('name')) {
            ...
            if (vehicle.getNumMods(tuningParams[key].modType) > 0 || tuningParams[key].ignoreModGetter) {
                // Формируем массив значений модификаций
                ...
                for (let i = -1; i < numMods; i++) {
                    let text;
                    if (data.hasOwnProperty("defaultModNames")) {
                        text = data.defaultModNames[i + 1];
                        if (!text) continue;
                    } else {
                        let label = vehicle.getModTextLabel(data.modType, i);
                        if (typeof label !== 'string' || !label || label === 'NULL') {
                            text = i != -1 ? `${data.name} ${i + 1}` : 'Нет';
                        } else {
