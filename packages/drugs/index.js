...
module.exports = {
    ...
    async init() {
        try {
            ...
            console.error('[DRUGS] Ошибка при инициализации:', err);
        }
    },
    ...
    async loadCoordinatesFromDB() {
        try {
            ...
            console.error('[DRUGS] Ошибка при загрузке координат:', err);
        }
    },
    ...
    async loadDrugsStashFromDB() {
        try {
            ...
            if (!this.drugsStash) {
                console.warn('[DRUGS] Не найдено хранилище наркотиков в базе! Создаю пустое.');
                this.drugsStash = await db.Models.DrugsStash.create({
                    id: 1,
                    resources: 0,
                    resourcesMax: 1000
                });
            ...
            console.error('[DRUGS] Ошибка при загрузке хранилища:', err);
        }
    },
    ...
    createSellMarker() {
        ...
        this.drugsStashLabel = mp.labels.new('', new mp.Vector3(sellPoint.x, sellPoint.y, sellPoint.z), {
            los: false,
            font: 0,
            drawDistance: 10,
    ...
    drugsStashLabelUpdate() {
        if (!this.drugsStash || !this.drugsStashLabel) return;
