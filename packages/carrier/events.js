...
module.exports = {
    ...
    "carrier.vehicle.buy": (player) => {
        ...
        money.removeCash(player, carrier.vehPrice, (res) => {
            ...
            if (carrier.isOilTanker(veh)) {
                veh.oil = {
                    max: carrier.getBarrelsMax(player),
                    current: 0
                }
                carrier.updateOilTankerLabel(veh);
                player.call(`prompt.waitShowByName`, [`oiltanker_job`]);
            } else {
    ...
    // НОВЫЙ ОБРАБОТЧИК ДЛЯ ПРОВЕРКИ НЕФТЕВОЗА
    "carrier.checkOilTanker": (player, remoteId) => {
        const canCollect = carrier.checkOilTankerByRemoteId(remoteId);
        player.call("carrier.checkOilTanker.result", canCollect);
    },
}